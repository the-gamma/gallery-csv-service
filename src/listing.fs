...
let handleRequest root (files:CsvFile[]) = 
  ...
  choose [
    path "/providers/listing/" >=> request (fun r ->
      Serializer.returnMembers [        
        Member.Property("by date", Result.Nested("date/"), [])
        Member.Property("by tag", Result.Nested("tag/"), [])
      ])
    path "/providers/listing/date/" >=> request (fun r ->
      Serializer.returnMembers [        
        for y, m in dates ->
          let name = System.Globalization.DateTimeFormatInfo.InvariantInfo.GetMonthName(m)
          Member.Property(name + " " + string y, Result.Nested("date/" + string y + "-" + string m + "/"), [])
      ])
    ...
    pathScan "/providers/listing/date/%d-%d/" (fun (y, m) ->
      Serializer.returnMembers [
        for file in files do
          ...
          if show then yield Member.Property(file.title, Result.Provider("pivot", root + "/providers/csv/" + file.id), [])
      ])
